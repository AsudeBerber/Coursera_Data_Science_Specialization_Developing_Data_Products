...
library(rCharts)
data(ISO_3166_1)
USState<-read.xlsx("Downloads/USStates.xlsx",header=TRUE,sheetIndex=1)
USStatesData<-melt(USState,id.vars="State")
names(USStatesData)<-c("State","Indicator","Value")
BMICrude<-read.csv("Downloads/dataverbose.csv")
BMICat<-read.csv("Downloads/ObservationDataiijxxz.csv")
BMICat$Year<-as.numeric(format(as.Date(as.character(BMICat$Date),format="%d/%m/%Y"),"%Y"))
shinyServer(
  function(input, output) {
    
    output$weight <- renderPrint({input$weightM})
    output$height <- renderPrint({input$heightM})
    ...
    output$BMIcrude <- renderText({
      input$goButton
      isolate(BMICrd(input$country,input$gender))
    })
    output$BMIcattable = renderDataTable({
      input$goButton
      isolate(BMICategories(input$country))
    })
    output$PlotYear = renderText({
      input$goButton
